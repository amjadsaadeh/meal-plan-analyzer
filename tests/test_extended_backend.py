...
class TestExtendedMealPlanLogic:
    def test_meal_plan_summary_with_missing_nutrients(self, authenticated_client):
        ...
        food_a = Food.objects.create(bls_code="A", name="Protein Heavy", protein_in_g_per_100g=10.0, energy_in_kj_per_100g=0, energy_in_kcal_per_100g=0)
        food_b = Food.objects.create(bls_code="B", name="Protein Light", protein_in_g_per_100g=0.0, energy_in_kj_per_100g=0, energy_in_kcal_per_100g=0)
        
        plan = MealPlan.objects.create(name="Balanced Plan", visible_nutrients=["protein_in_g"])
        ...
        MealPlanFood.objects.create(meal_plan_day=day, food=food_a, amount_in_g=100.0, meal_type="breakfast")
        MealPlanFood.objects.create(meal_plan_day=day, food=food_b, amount_in_g=100.0, meal_type="lunch")
        
        ctx = get_meal_plan_context(plan.pk)
        prot_summary = next(n for n in ctx["summary_nutrients"] if n["label"] == "Protein")
    ...
    def test_search_multiple_terms_relevance(self, authenticated_client):
        ...
        """
        Food.objects.create(bls_code="F1", name="Red Apple", energy_in_kj_per_100g=0, energy_in_kcal_per_100g=0)
        Food.objects.create(bls_code="F2", name="Green Apple", energy_in_kj_per_100g=0, energy_in_kcal_per_100g=0)
        Food.objects.create(bls_code="F3", name="Red Tomato", energy_in_kj_per_100g=0, energy_in_kcal_per_100g=0)
    ...
    def test_apply_threshold_preset(self, authenticated_client):
        ...
        preset = ThresholdPreset.objects.create(
            name="Weight Gain",
            energy_in_kcal_min=3000,
            energy_in_kcal_max=4000
        )
        plan = MealPlan.objects.create(name="My Plan")
        
        # Simulate frontend applying preset
        thresholds = {
            "energy_in_kcal": {"min": preset.energy_in_kcal_min, "max": preset.energy_in_kcal_max}
        }
        ...
        response = authenticated_client.patch(api_url, {"thresholds": thresholds}, format='json')
    ...
    def test_export_name_persistence_in_context(self, authenticated_client):
        ...
        """
        food = Food.objects.create(bls_code="E1", name="Original Name", energy_in_kj_per_100g=0, energy_in_kcal_per_100g=0)
