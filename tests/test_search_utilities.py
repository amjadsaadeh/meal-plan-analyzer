import pytest
from meals.views import normalize_umlauts, _umlaut_search_variants, parse_food_search, get_food_search_query, get_food_ids_by_alias
...
def test_get_food_search_query():
    ...
    Food.objects.create(bls_code="A1", name="Apple", energy_in_kj_per_100g=0, energy_in_kcal_per_100g=0)
    assert Food.objects.filter(query).count() == 1

@pytest.mark.django_db
def test_get_food_ids_by_alias():
    f = Food.objects.create(bls_code="A2", name="Apple", energy_in_kj_per_100g=0, energy_in_kcal_per_100g=0)
...
def test_alias_cache_invalidation():
    ...
    f = Food.objects.create(bls_code="A3", name="Banana", energy_in_kj_per_100g=0, energy_in_kcal_per_100g=0)
