...
- [ ] **K8S-03**: ConfigMap (`meal-plan-analyzer-config`) includes `CELERY_BROKER_URL=redis://redis:6379/0`, `REDIS_URL=redis://redis:6379/1`, and `SITE_BASE_URL=http://localhost:8000`
- [ ] **K8S-04**: Celery worker runs as a sidecar container (`worker`) in the `meal-plan-analyzer` pod, mounting the `media` PVC, using the same image and env as the web container
