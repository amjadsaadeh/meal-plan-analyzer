...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('file_path', type=str, help='Path to xlsx/zip file or URL (http/https)')

    def _find_xlsx_in_zip(self, zip_path: str, extract_dir: str) -> str | None:
        with zipfile.ZipFile(zip_path, 'r') as zf:
            xlsx_files = [f for f in zf.namelist() if f.endswith('.xlsx') and 'Daten' in os.path.basename(f)]
            if not xlsx_files:
                ...
                return None
            if len(xlsx_files) > 1:
                self.stdout.write(self.style.WARNING(f'Multiple "Daten" xlsx files found: {xlsx_files}'))
            target = xlsx_files[0]
    ...
    def _get_xlsx_path(self, source: str):
        ...
        try:
            ...
            if is_zip:
                ...
                if not xlsx_path:
                    raise ValueError('No xlsx file with "Daten" in name found in ZIP archive.')
                self.stdout.write(self.style.SUCCESS(f'Found data file: {os.path.basename(xlsx_path)}'))
                yield xlsx_path
            else:
                yield file_path
        ...
        try:
            with self._get_xlsx_path(source) as xlsx_path:
                try:
                    ...
                    self.stderr.write(self.style.ERROR(f'Invalid file format. Expected .xlsx file.'))
                    return
                except Exception as e:
                    self.stderr.write(self.style.ERROR(f'Failed to parse Excel file: {str(e)}'))
                    return
